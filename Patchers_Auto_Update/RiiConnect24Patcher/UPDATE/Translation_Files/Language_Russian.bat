...
set string453=У разработчиков News Channel'а была идея сделать так чтобы ведущий новостей был Mii папы, но это не случилось наверное потому-что некоторые истории на этом канале не совсем подходят для детей.
set string454=Everybody Votes Channel изначально назывался Questionnaire Channel, а позже Citizens Vote Channel.
set string455=У Forecast Channel'а был "прачечный индекс" (показывающий насколько подходяще сушить вещи на улице) и "счётчик пыльцы" в Японской версии канала.
